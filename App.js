...
const MongoStore = require('connect-mongo'); // NO (session) in v4+
...
let sessionOptions = session({
    ...
    store: MongoStore.create({
        mongoUrl: process.env.CONNECTIONSTRING || ' mongodb+srv://pady:Lisa2020@cluster0.wyr0riq.mongodb.net/ComplexApp?retryWrites=true&w=majority',
        dbName: 'ComplexApp'
    }),
    resave: false,
...
app.use(function(err, req, res, next){
    if(err){
        if(err.code == "EBADCSRFTOKEN"){
            req.flash('error',"Cross Site Request forgery detected.")
